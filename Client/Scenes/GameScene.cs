...
namespace Client.Scenes
{
    public sealed class GameScene : DXScene
    {
            ...
            if (displayInfo.RequiredAmount > 0)
            {
                ...
                switch (displayInfo.RequiredType)
                {
                    ...
                    case RequiredType.FamePoints:
                        text = $"Required FamePoints: {MouseItem.Info.RequiredAmountFP}";
                        if (User.FPBox.Amount < MouseItem.Info.RequiredAmountFP)
                            colour = Color.Red;
                        break;
        ...
        public bool CanUseItem(ClientUserItem item)
        {
            ...
            switch (item.Info.RequiredType)
            {
                case RequiredType.Level:
                    ...
                    break;
                case RequiredType.FamePoints:
                    if (User.FPBox.Amount < item.Info.RequiredAmountFP) return false;
                    break;
                case RequiredType.MaxLevel:
                    if (User.Level > item.Info.RequiredAmount || User.Stats[Stat.Rebirth] > 0) return false;
                    break;
        ...
        public void CurrencyChanged()
        {
            ...
            MarketPlaceBox.FPBox.Value = User.FPBox.Amount;
